...
object DataFramesBasics extends App {
  ...
  val smartphones2 = Seq(("Iphone", "11 Pro", 5.5, 32.0),
    ("Samsung", "20", 6.2, 22.5),
    ("Xiaomi", "Test", 6.7, 34.4)).toDF("make", "model", "screen dimension", "camera megapixels")
  ...
  private val schemaMoves: StructType = secondDF.schema

  val movesDFWithSchema = spark.read
    .format("json")
    .schema(schemaMoves)
    .load("src/main/resources/data/movies.json")

  movesDFWithSchema.show()
  movesDFWithSchema.printSchema()
  println(s"Number of rows: ${movesDFWithSchema.count()}")
